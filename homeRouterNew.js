...
router.get('/tutorial', ensureLogin, async (req, res) => {
    ...
    if (!DEVELOPMENT) {
        ...
        if (accountValueTemp.toString() == "null" || isNaN(accountValueTemp.toString())) {
            ...
            updateInformation(req.user.id, accountValueTemp, req.user.buyingPower);
            if (currency != 'USD') {
                res.render('index', {
...
function updateInformation (id, value, buyingPower) {
    User.findByIdAndUpdate(id, {  
        ...
        if (err){
            ...
            console.log("User " + id + " homepage loaded");
        }
    });
}
